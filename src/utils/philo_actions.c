...
void	*philo_routine(void *arg)
{
	...
	{
		pthread_mutex_lock(philo->left_fork);
		print_message(philo, "has taken a fork", "blue", philo->data->dead_flag);
	...
	while (!is_dead(philo->data))
	{
		...
		print_message(philo, "is sleeping", "yellow", philo->data->dead_flag);
		precise_usleep(philo->data->t_sleep, philo->data);
		print_message(philo, "is thinking", "magenta", philo->data->dead_flag);
	}
	return (NULL);
}
